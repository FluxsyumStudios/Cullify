...
/**
 * Custom in-game results screen for the Cullify benchmark (Minecraft 1.20.1 / Forge).
...
public class BenchmarkResultsScreen extends Screen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        // Header Title
        graphics.drawCenteredString(this.font, "§2§lCullify §a§lBenchmark Results", centerX, mainY1 + 10, 0xFFFFFFFF);
        ...
        graphics.drawString(this.font, "§6§nComparative Stats", leftX + 8, statsY + 6, 0xFFFFFFFF, false);

        int rowY = statsY + 22;
        graphics.drawString(this.font, "Average FPS:", leftX + 8, rowY, 0xFFE0E0E0, false);
        graphics.drawString(this.font, String.format("§7Without: §f%.1f", this.metricsWithout.avgFps), leftX + 16, rowY + 10, 0xFFFFFFFF, false);
        graphics.drawString(this.font, String.format("§aWith:    §f%.1f", this.metricsWith.avgFps), leftX + 16, rowY + 20, 0xFFFFFFFF, false);
        graphics.drawString(this.font, String.format("§eGain:    §a%+.1f%%", gainAvg), leftX + 16, rowY + 30, 0xFFFFFFFF, false);

        rowY += 45;
        graphics.drawString(this.font, "1% Low FPS (Stability):", leftX + 8, rowY, 0xFFE0E0E0, false);
        graphics.drawString(this.font, String.format("§7Without: §f%.1f", this.metricsWithout.low1Fps), leftX + 16, rowY + 10, 0xFFFFFFFF, false);
        graphics.drawString(this.font, String.format("§aWith:    §f%.1f", this.metricsWith.low1Fps), leftX + 16, rowY + 20, 0xFFFFFFFF, false);
        graphics.drawString(this.font, String.format("§eGain:    §a%+.1f%%", gain1), leftX + 16, rowY + 30, 0xFFFFFFFF, false);
        ...
        graphics.drawString(this.font, "§d§nFPS Stability Graph", rightX1, statsY + 6, 0xFFFFFFFF, false);
        ...
        // Chart Labels
        graphics.drawString(this.font, String.format("%.0f", this.maxFpsValue), rightX1 + 4, chartY1 + 3, 0x66FFFFFF, false);
        graphics.drawString(this.font, String.format("%.0f", this.minFpsValue), rightX1 + 4, chartY2 - 11, 0x66FFFFFF, false);

        // Bottom Legend under chart
        int legendY = chartY2 + 4;
        graphics.fill(rightX1, legendY + 3, rightX1 + 10, legendY + 5, 0xFFE74C3C);
        graphics.drawString(this.font, "Without", rightX1 + 13, legendY, 0x88FFFFFF, false);

        graphics.fill(rightX1 + 70, legendY + 3, rightX1 + 80, legendY + 5, 0xFF2ECC71);
        graphics.drawString(this.font, "With", rightX1 + 83, legendY, 0x88FFFFFF, false);
    ...
    /**
     * In 1.20.1, renderBackground only takes the GuiGraphics parameter.
     * Override to avoid drawing the blurred background while in-game.
     */
    @Override
    public void renderBackground(GuiGraphics graphics) {
        // Draw a dim overlay instead of the full blurred background to avoid covering the world view
